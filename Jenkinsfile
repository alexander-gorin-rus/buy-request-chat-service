...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/chat-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/chat-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/chat-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make chat-service-restart && make nginx-restart \
                        && make chat-service-logs-without-tail \
